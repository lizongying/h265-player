//Decoder states.
window.decoderStateIdle = 0;
window.decoderStateInitializing = 1;
window.decoderStateReady = 2;
window.decoderStateFinished = 3;

//Player states.
window.playerStateIdle = 0;
window.playerStatePlaying = 1;
window.playerStatePausing = 2;

//Constant.
window.maxBufferTimeLength = 1.0;
window.downloadSpeedByteRateCoef = 2.0;
...
Player.prototype.initDownloadWorker = function() {
    var self = this;
    this.downloadWorker = new Worker(chrome.runtime.getURL('downloader.js'));
...
Player.prototype.initDecodeWorker = function() {
    var self = this;
    this.decodeWorker = new Worker(chrome.runtime.getURL('decoder.js'));
...
Player.prototype.play = function(
    url, canvas, callback, waitHeaderLength, isStream) {
...
Player.prototype.stop = function() {
    ...
    if (this.timeLabel) {
        this.timeLabel.innerHTML = this.formatTime(0) + '/' +
...
Player.prototype.fullscreen = function() {
    ...
    return this.webglPlayer;
};
Player.prototype.exitscreen = function() {
    if (this.webglPlayer) {
        this.webglPlayer.exitscreen();
    }
    return this.webglPlayer;
};
...
Player.prototype.onGetFileInfo = function(info) {
    ...
    this.logger.logInfo(
        'Got file size rsp:' + info.st + ' size:' + info.sz + ' byte.');
    if (info.st == 200) {
...
Player.prototype.onVideoParam = function(v) {
    ...
    this.logger.logInfo(
        'Video param duation:' + v.d + ' pixFmt:' + v.p + ' width:' + v.w +
        ' height:' + v.h + '.');
    this.duration = v.d;
    this.pixFmt = v.p;
    if (v.w / v.h > 2) {
        this.canvas.style.width = 800 + 'px';
        this.canvas.style.height = v.h / v.w * 800 + 'px';
    } else {
        this.canvas.style.height = 400 + 'px';
        this.canvas.style.width = v.w / v.h * 400 + 'px';
    }
    ...
    */
    ...
    this.logger.logInfo(
        'Byte rate:' + byteRate + ' target speed:' + targetSpeed +
        ' chunk interval:' + this.chunkInterval + '.');
};
...
Player.prototype.onAudioParam = function(a) {
    ...
    this.logger.logInfo(
        'Audio param sampleFmt:' + a.f + ' channels:' + a.c + ' sampleRate:' +
        a.r + '.');
    ...
    switch (sampleFmt) {
        ...
        default:
            this.logger.logError(
                'Unsupported audio sampleFmt ' + sampleFmt + '!');
    }
...
Player.prototype.bufferFrame = function(frame) {
    ...
    if (this.getBufferTimerLength() >= maxBufferTimeLength ||
        this.decoderState ==
...
Player.prototype.onRequestData = function(offset, available) {
    if (this.justSeeked) {
        this.logger.logInfo(
            'Request data ' + offset + ', available ' + available);
        if (offset == -1) {
            // Hit in buffer.
            let left = this.fileInfo.size - this.fileInfo.offset;
            if (available >= left) {
...
Player.prototype.renderVideoFrame = function(data) {
    this.webglPlayer.renderFrame(data, this.videoWidth, this.videoHeight,
        this.yLength, this.uvLength);
};
...
Player.prototype.downloadOneChunk = function() {
    ...
    if (len > this.fileInfo.chunkSize) {
        console.log('Error: request len:' + len + ' > chunkSize:' +
...
Player.prototype.formatTime = function(s) {
    var h = Math.floor(s / 3600) < 10 ? '0' + Math.floor(s / 3600) : Math.floor(
        s / 3600);
    var m = Math.floor((s / 60 % 60)) < 10
        ? '0' + Math.floor((s / 60 % 60))
        : Math.floor((s / 60 % 60));
    var s = Math.floor((s % 60)) < 10 ? '0' + Math.floor((s % 60)) : Math.floor(
        (s % 60));
    return result = h + ':' + m + ':' + s;
};
