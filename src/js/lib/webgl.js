...
WebGLPlayer.prototype.initGL = function(options) {
    ...
    var vertexPositionAttribute = gl.getAttribLocation(program,
        'aVertexPosition');
...
WebGLPlayer.prototype.fullscreen = function() {
    let bw = parseInt(document.body.clientWidth);
    let bh = parseInt(document.body.clientHeight);
    document.getElementById(
        'videoPlayer').style.width = bw + 'px';
    let nodes = document.getElementsByClassName('canvasDiv');
    for (let i = 0; i < nodes.length; i++) {
        nodes[i].style.width = bw + 'px';
        nodes[i].style.height = bh + 'px';
    }
    var canvas = this.canvas;

    let cw = parseInt(this.canvas.style.width);
    let ch = parseInt(this.canvas.style.height);
    let cr = cw / ch;
    if (cr > 2) {
        this.canvas.style.width = bw + 'px';
        this.canvas.style.height = 1 / cr *
            bw + 'px';
    } else {
        this.canvas.style.height = bh + 'px';
        this.canvas.style.width = cr *
            bh + 'px';
    }
    return;
...
WebGLPlayer.prototype.exitscreen = function() {
    document.getElementById(
        'videoPlayer').style.width = 800 + 'px';
    let nodes = document.getElementsByClassName('canvasDiv');
    for (let i = 0; i < nodes.length; i++) {
        nodes[i].style.width = 800 + 'px';
        nodes[i].style.height = 400 + 'px';
    }
    var canvas = this.canvas;
